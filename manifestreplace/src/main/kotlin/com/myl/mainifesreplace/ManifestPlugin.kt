    ...
    override fun apply(project: Project) {
        ...
        if (appPlugin != null) {
            ...
            android.applicationVariants.all { it ->
                ...
                val appId = it.getGenerateBuildConfigProvider().get().applicationId.get()
                val newManifest = ComponentsGenerator.generateComponent(appId, config)
                it.outputs.forEach {
//                    println(it.processManifestProvider.get().reportFile.get())
                    val manifestOutFile = it.processManifestProvider.get().outputs.files.singleFile
